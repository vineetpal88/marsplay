...
public class LimitCacheSizeGlideModule implements GlideModule {
    ...
    public void applyOptions(Context context, GlideBuilder builder) {
        //if (AppController.getContext().) return; // NOTE: StatFs will crash on robolectric.

        double totalGiB = getTotalBytesOfInternalStorage() / 1024.0 / 1024.0 / 1024.0;
        Log.d("chk", "Internal Storage Size: %.1fGiB"+ totalGiB);
        if (totalGiB < SMALL_INTERNAL_STORAGE_THRESHOLD_GIB) {
            Log.d("chk","Limiting image cache size to " + DISK_CACHE_SIZE_FOR_SMALL_INTERNAL_STORAGE_MIB + "MiB");
